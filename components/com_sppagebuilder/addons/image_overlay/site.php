...
class SppagebuilderAddonImage_overlay extends SppagebuilderAddons{
	...
	public function render() {
		...
		if(strpos($image, "http://") !== false || strpos($image, "https://") !== false){
			...
			$image_link= JURI::base(true) . '/' . $image;
		}
		...
		if(($title || $sub_title) && $title_subtitle_position){
			$output .= '<div class="overlay-image-title">';
				$output .= '<'.$heading_selector.' class="sppb-addon-title">';
					...
					if($title_icon){
						$output .= '<i class="fa ' . $title_icon . '"></i>';
					}
					$output .= $title;
					if($title_link){
						$output .= '</a>';
				...
				if($sub_title){
					$output .= '<'.$subtitle_heading_selector.' class="sppb-addon-subtitle">';
						if($sub_title_icon){
							$output .= '<i class="fa ' . $sub_title_icon . '"></i>';
						}
						$output .= $sub_title;
	...
	public function css() {
		...
		$subtitle_style .= (isset($settings->sub_title_margin) && $settings->sub_title_margin) ? 'margin:'.$settings->sub_title_margin.';' : '';
		...
		if($overlay_type!=='gradient' && $overlay_color){
		...
		if($overlay_hover_type!=='none'){
			if($overlay_hover_type!=='gradient' && $overlay_hover_color){
		...
		//Button Margin
		$button_margin = (isset($settings->button_margin) && $settings->button_margin) ? $settings->button_margin : '';
        $button_margin_sm = ((isset($settings->button_margin_sm)) && $settings->button_margin_sm) ? $settings->button_margin_sm : '';
		$button_margin_xs = ((isset($settings->button_margin_xs)) && $settings->button_margin_xs) ? $settings->button_margin_xs : '';
		...
		$content_padding_sm = (isset($settings->content_padding_sm) && $settings->content_padding_sm) ? $settings->content_padding_sm : '';
		...
		$content_padding_xs = (isset($settings->content_padding_xs) && $settings->content_padding_xs) ? $settings->content_padding_xs : '';
	...
	public static function getTemplate() {
		...
		<# }
		if(data.overlay_type!=="gradient" && data.overlay_color){
		#>
		...
		<# }
			...
			let button_margin_sm = (!_.isEmpty(data.button_margin) && data.button_margin.sm) ? data.button_margin.sm : "";
			let content_padding_sm = (!_.isEmpty(data.content_padding) && data.content_padding.sm) ? data.content_padding.sm : "";
			
		if(image_height_sm || title_margin_sm || subtitle_fontsize_sm || sub_title_margin_sm || button_margin_sm || content_padding_sm){
		#>
		...
		let button_margin_xs = (!_.isEmpty(data.button_margin) && data.button_margin.xs) ? data.button_margin.xs : "";
		let content_padding_xs = (!_.isEmpty(data.content_padding) && data.content_padding.xs) ? data.content_padding.xs : "";

		if(image_height_xs || title_margin_xs || subtitle_fontsize_xs || button_margin_xs || sub_title_margin_xs || content_padding_xs){
		#>
