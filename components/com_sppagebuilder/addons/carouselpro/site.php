...
class SppagebuilderAddonCarouselpro extends SppagebuilderAddons {
	...
	public function render() {
		...
		if($arrows) {
			...
			$output	.= '<a href="#sppb-carousel-'. $this->addon->id .'" class="sppb-carousel-arrow left sppb-carousel-control" data-slide="prev"><i aria-hidden="true" aria-label="'.JText::_('COM_SPPAGEBUILDER_ARIA_PREVIOUS').'" class="fa '.$left_arrow.'"></i></a>';
			$output	.= '<a href="#sppb-carousel-'. $this->addon->id .'" class="sppb-carousel-arrow right sppb-carousel-control" data-slide="next"><i aria-hidden="true" aria-label="'.JText::_('COM_SPPAGEBUILDER_ARIA_NEXT').'" class="fa '.$right_arrow.'"></i></a>';
			if($arrow_position !== 'default'){
				$output  .= '</div>';
				$output  .= '</div>';
				$output  .= '</div>';
	...
	public function css() {
        ...
        $arrow_style .= (isset($settings->arrow_margin) && $settings->arrow_margin) ? "margin: " . $settings->arrow_margin . ";" : "";
		...
		$item_padding_sm = (isset($settings->item_padding_sm) && $settings->item_padding_sm) ? $settings->item_padding_sm : '';
		...
		$item_padding_xs = (isset($settings->item_padding_xs) && $settings->item_padding_xs) ? $settings->item_padding_xs : '';
		...
		<div id="sppb-carousel-{{data.id}}" class="sppb-carousel sppb-carousel-pro sppb-slide {{ data.class }}" data-interval="{{ interval }}" {{{ autoplay }}}>
			...
			<div class="sppb-carousel-inner {{ data.alignment }}">
				<# _.each(data.sp_carouselpro_item, function (carousel_item, key){ #>
					...
					<# if(typeof(data.carousel_height) == "undefined"){
						if(carousel_item.bg){ #>
